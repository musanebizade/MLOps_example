...
def train_model(X_train, y_train, preprocessor, model_type="rf"):
    ...
    print(
        f"{model_type.upper()} CV Accuracy: {np.mean(scores):.4f} ± {np.std(scores):.4f}"
    )
...
def hypothesis_testing(rf_scores, xgb_scores):
    ...
    if p_value < 0.05:
        better_model = (
            "Random Forest" if np.mean(rf_scores) > np.mean(xgb_scores) else "XGBoost"
        )
...
def save_best_model(rf_pipeline, xgb_pipeline, rf_scores, xgb_scores):
    ...
    best_model = (
        rf_pipeline if np.mean(rf_scores) > np.mean(xgb_scores) else xgb_pipeline
    )
...
def main():
    ...
    # Train models
    rf_pipeline, rf_scores = train_model(
        X_train, y_train, preprocessor, model_type="rf"
    )
    xgb_pipeline, xgb_scores = train_model(
        X_train, y_train, preprocessor, model_type="xgb"
    )

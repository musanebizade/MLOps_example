...
with st.sidebar:
    ...
    try:
        ...
        if health_response.status_code == 200:
            ...
            st.info(f"Model Status: {'Loaded' if health_data.get('model_loaded') else 'Not Loaded'}")
        else:
...
if uploaded_file is not None:
    try:
        ...
        with col_status2:
            if extra_cols:
                st.info(f"📎 Extra columns found: {', '.join(extra_cols[:3])}{'...' if len(extra_cols) > 3 else ''}")
        
        # Data preview section
        if show_data_preview:
            st.subheader("👀 Data Preview")
            
            preview_tabs = st.tabs(["📋 Data Sample", "📊 Column Info", "🔢 Summary Stats"])
        ...
        with prediction_col1:
            ...
            if not missing_cols:
                if st.button("🚀 Get Predictions", type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            # Display results
                            st.success(f"✅ Successfully generated {num_predictions} predictions!")
                            
                            # Results tabs
                            result_tabs = st.tabs(["📊 Results Overview", "📈 Visualization", "💾 Download Results"])
                            ...
                            with result_tabs[0]:
                                ...
                                with col_stat2:
                                    if pred_series.dtype in ['int64', 'float64']:
                                        st.metric("Mean Prediction", f"{pred_series.mean():.2f}")
                                    else:
                                ...
                                with col_stat3:
                                    if pred_series.dtype in ['int64', 'float64']:
                                        st.metric("Min Prediction", f"{pred_series.min():.2f}")
                                    else:
                                        most_common = pred_series.mode().iloc[0] if not pred_series.empty else "N/A"
                                        st.metric("Most Common", str(most_common))
                                
                                with col_stat4:
                                    if pred_series.dtype in ['int64', 'float64']:
                                        st.metric("Max Prediction", f"{pred_series.max():.2f}")
                                    else:
                            ...
                            with result_tabs[1]:
                                ...
                                if is_numeric:
                                    ...
                                    # Histogram
                                    ax1.hist(pred_numeric.dropna(), bins=20, color='#667eea', alpha=0.7, edgecolor='black')
                                    ...
                                    # Box plot
                                    ax2.boxplot(pred_numeric.dropna(), patch_artist=True,
                                    ...
                                    if len(predictions) > 1:
                                        ...
                                        ax3.plot(range(len(pred_numeric)), pred_numeric, 
                                                color='#667eea', linewidth=2, marker='o', markersize=3)
                                ...
                                value_counts_df = pd.DataFrame({
                                    ...
                                    'Percentage': (value_counts.values / len(predictions) * 100).round(2)
                                })
                            ...
                            with result_tabs[2]:
                                ...
                                st.info("💡 Results include original data plus predictions and timestamp")
                            ...
                            with st.expander("🔧 Troubleshooting"):
                                ...
                                st.write("- Check that all required columns are present")
                ...
                st.info("Please upload a CSV file with all required columns listed in the sidebar")
                            ...
                            with st.expander(f"📊 {col} Analysis"):
                                ...
                                with col_analysis2:
                                    if unique_vals <= 10:
                                        ...
                                        st.write(f"**Sample values:** {', '.join(map(str, col_data.unique()[:5]))}")
